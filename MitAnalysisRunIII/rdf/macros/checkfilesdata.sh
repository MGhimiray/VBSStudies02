...
outdir="/mnt/home/mghimiray/VBSStudies/Outputs_VBS/DataDriven/${whichana}/histo/fillhisto_sswwAnalysis"
...
for tag in "${tags[@]}"; do
  for sample in ${samples[$tag]}; do
    for job in $jobs_list; do
      f="$outdir/fillhisto_sswwAnalysis${whichana}_sample${sample}_year${tag}_job${job}.root"
      (( total_expected++ ))
      if [[ -s "$f" ]]; then
        (( total_found++ ))
      else
  ...
  while read -r y wa tag sample job path; do
    ...
    if [[ -z "${DRYRUN:-}" ]]; then
      sbatch -p INTEL_HASWELL --time=70:00:00 --cpus-per-task=1 --nodes=1 \
             --job-name="vbs_${year}_${sample}_${job}" \
